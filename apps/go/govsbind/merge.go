...
func MergedKeybindings(file1, file2 string) (MergeResult, error) {
	...
	if err != nil {
		return MergeResult{}, fmt.Errorf("failed to read keybindings 1 from %s: %w", file1, err)
	}
	b2, err := ReadKeybindings(file2)
	if err != nil {
		return MergeResult{}, fmt.Errorf("failed to read keybindings 2 from %s: %w", file2, err)
	}
	// Compare the keybindings in the two files
	result, info := MergeBindings(b1, b2)

	contents, err := result.Marshal("", "   ")
	if err != nil {
